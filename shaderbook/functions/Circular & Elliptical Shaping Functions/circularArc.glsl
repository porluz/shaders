...
This function defines a circular arc which passes through a user-specified point in the unit square. Unfortunately, not every location in the unit square lends itself to defining a circle which also is confined to the unit square; the user-supplied point must inhabit a zone close to the main (Identity) diagonal. This pattern is adapted from 
Paul Bourke's Equation of a Circle From 3 Points. */
//---------------------------------------------------------
...
float circularArcThroughAPoint (inout float x, inout float a, inout float b){  
...
void main() {
    ...
    float a = 0.1;
    float b = 0.4;
    float y = circularArcThroughAPoint(
        st.x, a, b);
