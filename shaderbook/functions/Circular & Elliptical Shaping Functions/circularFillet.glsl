...
void computeFilletParameters (
  ...
  float arcStart = atan(gv1y,gv1x); // flip
  //float arcStart = atan(gv1x, gv1y); // flip
  float arcAngle = 0.0;
...
float circularFillet (float x, float a, float b, float R){
  ...
  computeFilletParameters (0., 0., newa, newb, newa, newb, 1., 1., R);
